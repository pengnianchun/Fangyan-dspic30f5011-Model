...
unsigned int ADR2 = 0; //第二路传感器电阻值����������ֵ
...
void get_hw_address(void) {
    ...
    for (i = 0; i < 3; i++) {                //硬线读三次
        ...
        else if ((KL13 == 0) && (KL14 == 1)) dump[i] = 3; //顶模块
        else if ((KL13 == 1) && (KL14 == 0)) dump[i] = 2; //后模块
        else dump[i] = 0;
    }
...
void system_Init(void) {
    ...
    if (g_address == 3)//顶模块
        //KL2 控地开关前端带负载，会导致负载工作，故将KL2改车上拉状态
        mc33978_write(0x07, 0x00, 0x2D, 0x20); //0x2D,SG13,SG11,SG10,SG8=1;0x22 SG5=1; SG1=0 ; 
...
void can_send_message_key(void) {
    ...
    if(msg.data[1]&0x40)msg.data[1] &=0xBF;
    else                msg.data[1] |=0x40;
...
void can_send_message_ADR(void) {
    ...
    msg.data[0] = (unsigned char) (ADR1/10); //ADR1信号
    msg.data[1] = (unsigned char) ((ADR1/10) >> 8);

    msg.data[2] = (unsigned char) (ADR2/10); //ADR2信号
    msg.data[3] = (unsigned char) ((ADR2/10) >> 8);
...
unsigned int  ICcan_count = 0; //仪表CAN
...
void rain_wape(void) {
    ...
    //雨刮无操作，需要停止工作
    if (!K_ON || (!RAIN_SLOW && !RAIN_HIGH && !RAIN_INTERVAL && !RAIN_SPRAY && (wash_time == 0))) {
        /*if(ADD == 0 && wipe_stop_time > 0){//雨刮半途掉电，上电可自动归位
            OUT1 = 0;
            OUT2 = 1;
            OUT3 = 0;
            PO16 = 1;
        ...
        if (wipe_stop_time == 0) { //雨刮操作开关断开 20秒内强制关闭雨刮 保护雨刮电机
            OUT1 = 0;
            OUT2 = 0;
            OUT3 = 0;
            PO16 = 1;
    ...
    if (ADD) {
        if ((OUT1 == 0) && (OUT2 == 0)) {
            if (DELAY_END == 0) {
                DELAY_END = 1;
                wipe_wpp_time = 15;//打开放电延时时间为30-10=20ms,实际测量发现输出关短有10ms延时.因为Pout输出函数在大循环里，10ms执行一次
            }
            if (wipe_wpp_time == 0) {
                POWPP = 1;
                DELAY_END = 0;
                ...
                if (message.id == 0x610) {
                    ...
                    if (message.data[0] == 0x55) {
                        //if (message.data[1] == g_address) {
                            gPout.BYTES[0] = message.data[1];
                            gPout.BYTES[1] = message.data[2];
                        //}
                    } else if (message.data[0] == 0x56) {
                        //if (message.data[1] == g_address) {
                            e_hm_pulse = message.data[1]+((unsigned int) message.data[2] << 8);
                            EEROM_WriteTotalMiles();
                        //}
                    } else if (message.data[0] == 0x58) { //上电后更新
                        //if (message.data[1] == g_address) {
                            e_address = message.data[1];
                            EEROM_WriteAddress();
                        //}
                    } else if (message.data[0] == 0x59) {
                        //if (message.data[1] == g_address) {
                            e_total_miles = (unsigned char ) message.data[1]
                                           +((unsigned int ) message.data[2] << 8)
                                           +((unsigned long) message.data[3] << 16)
                                           +((unsigned long) message.data[4] << 24);
                            EEROM_WriteTotalMiles();
                        //}
                    }
                }
